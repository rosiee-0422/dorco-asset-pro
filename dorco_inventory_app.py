...
# ─────────────────────────────────────────────
# 4-1. 입고 캘린더 (독립 메뉴 + Inbound 페이지 하단 공용)
# ─────────────────────────────────────────────
def render_inbound_calendar(standalone: bool = True):
    ...
    if standalone:
        ...
        st.markdown('<p class="section-heading">📅 입고 캘린더 — 언제 무엇을 주문했는지 한눈에 확인</p>', unsafe_allow_html=True)
    ...
    key_suffix = "" if standalone else "_inline"

    nav1, nav2, nav3, nav4 = st.columns([1, 3, 1, 1.2])
    with nav1:
        if st.button("◀ 이전 달", use_container_width=True, key=f"cal_prev{key_suffix}"):
    ...
    with nav3:
        if st.button("다음 달 ▶", use_container_width=True, key=f"cal_next{key_suffix}"):
            st.session_state.cal_month += 1
            ...
            st.rerun()
    with nav4:
        if st.button("오늘로 이동", use_container_width=True, key=f"cal_today{key_suffix}"):
            st.session_state.cal_year = date.today().year
            st.session_state.cal_month = date.today().month
            st.rerun()
    ...
    cal_cat_sel = st.selectbox("대분류 필터", cal_cats, key=f"cal_cat_filter{key_suffix}")
...
with col_main:
    ...
    # ══════════════════════════════════════════
        ...
        render_inbound_calendar(standalone=True)
    ...
    # ══════════════════════════════════════════
        ...
        # ── 입고 등록 폼 아래에 이번 달 입고 캘린더 표시 (출고 모드에서는 숨김) ──
        if mode == "입고":
            st.divider()
            render_inbound_calendar(standalone=False)
    ...
    # ══════════════════════════════════════════
        ...
        if not uni_cats_present:
            ...
            # ── 지급 이력 ──
            st.markdown('<p class="section-heading">📋 지급 이력</p>', unsafe_allow_html=True)
            out_df = sb_select("inventory_out")
            if out_df.empty or "대분류" not in out_df.columns:
                st.info("지급 이력이 없습니다.")
            else:
                cat_out = out_df[out_df["대분류"] == sel_cat].copy()
                if cat_out.empty:
                    st.info("해당 분류의 지급 이력이 없습니다.")
                else:
                    cat_out["출고일자"] = pd.to_datetime(cat_out["출고일자"], errors="coerce").dt.strftime("%Y-%m-%d")
                    cat_out["출고수량"] = pd.to_numeric(cat_out["출고수량"], errors="coerce").fillna(0).astype(int)
                    hist = cat_out[["출고일자","품목","출고수량","비고"]].sort_values("출고일자", ascending=False)
                    hist.columns = ["지급일자","품목","지급수량","받는 사람 / 비고"]
                    st.dataframe(
                        hist, use_container_width=True, hide_index=True,
                        column_config={"지급수량": st.column_config.NumberColumn(format="%d")}
                    )

                    hist_csv = "\ufeff" + hist.to_csv(index=False)
                    st.download_button(
                        f"📥 [{sel_cat}] 지급 이력 CSV",
                        data=hist_csv.encode("utf-8"),
