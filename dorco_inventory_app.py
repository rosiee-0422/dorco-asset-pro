...
with col_main:
    ...
    # --------------------------------------------------
        ...
        # 항상 최신 파일 다시 로드
        stock_df = load_csv_safe(FILE_PATH)
        stock_df = prep_num_cols(stock_df, ["현재고", "안전재고"])
        ...
        if search_name:
            df_display = df_display[df_display["품목"].astype(str).str.contains(search_name, na=False)]
        ...
        if not info_df.empty:
            ...
            relevant_items = sorted(
                info_df[info_df["대분류"] == adj_cat]["품목"].dropna().astype(str).unique().tolist()
            )
        else:
            adj_cat = None
            relevant_items = []

        # 2. 품목 선택을 form 밖으로 이동
        adj_item = st.selectbox(
            "📦 품목 선택",
            relevant_items if relevant_items else ["품목 없음"],
            key="adj_item_select"
        )

        # 3. 선택된 품목의 기존 현재고 미리 조회
        target_row = stock_df[
            (stock_df["대분류"] == adj_cat) & (stock_df["품목"] == adj_item)
        ]
        current_val = int(target_row["현재고"].values[0]) if not target_row.empty else 0

        st.info(f"현재 선택 품목의 기존 현재고는 **{current_val}개** 입니다.")
                ...
                if not relevant_items or adj_item == "품목 없음":
                    ...
                    if not idx.empty:
                        ...
                        if int(new_qty) < old_qty:
                            stock_df.at[idx[0], "최근출고일"] = adj_date.strftime("%Y-%m-%d")
                        else:
                            stock_df.at[idx[0], "최근입고일"] = adj_date.strftime("%Y-%m-%d")
                ...
                if submit_btn:
                    ...
                    if not item_name:
                        ...
                        new_entry_info = {
                            ...
                            "수량단위": unit, "안전재고": safety_stock, "기본발주수량": default_order_qty, "비고": note
                        }
